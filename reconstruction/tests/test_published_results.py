...
class PublishedResultsTests(unittest.TestCase):
    def test_result_manifest(self):
        folder=ROOT.parent/'experiments'
        ...
        for line in records:
            ...
            listed.add(name)
        actual={p.relative_to(folder).as_posix() for sub in ['tape-z','synthetic','tape-frame']
                for p in (folder/sub/'results').rglob('*') if p.is_file() and '.mpl-cache' not in p.parts}
        self.assertEqual(listed,actual)

    def test_saved_pixels_and_source_bytes(self):
        with np.load(ROOT.parent/'experiments/tape-z/results/reconstruction.npz',allow_pickle=False) as d:
            metadata=json.loads(str(d['metadata_json']))
            original=(ROOT.parent/'experiments/tape-z/data/raw_data.txt').read_bytes()
    ...
    def test_dark_data_and_both_mappings(self):
        ...
        path=ROOT.parent/'experiments/tape-frame/data/raw_export.xls'
        ...
        for name in ['anchored','uniform']:
            with np.load(ROOT.parent/f'experiments/tape-frame/results/{name}/reconstruction.npz',allow_pickle=False) as d:
